...
input int  InpSmoothLen    = 64;     // tamanho da janela (amostras) do filtro (>=2)
input double InpCrossEqualEps = 0.00001;  // diff max para considerar linhas iguais (sem cruzamento)
input double InpGapPoints     = 0.0;      // gap extra (em pontos) entre Buy/Sell (aplica antes do filtro)
...
static const int  kWindowTicks = 2;   // N ticks (por lado)
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Pip/Tick Window (FFT smooth) GAP");
                ...
                const int &spread[])
{
   ...
   // Aplica gap extra entre as séries (em pontos -> pips), mantendo o centro
   if(val_buy != EMPTY_VALUE && val_sell != EMPTY_VALUE && InpGapPoints != 0.0)
   {
      double gap_pips = (InpGapPoints * _Point) / PipSize();
      double half = 0.5 * gap_pips;
      val_buy += half;
      val_sell -= half;
   }
   ...
   // Estado em tempo real (bar0) para entrada na barra atual
   double st0_buy = 0.0;
   double st0_sell = 0.0;
   if(rates_total >= 2)
   {
      ...
      double b1 = BufBuy[1];
      double s1 = BufSell[1];
      bool ok0 = (b0 != EMPTY_VALUE && s0 != EMPTY_VALUE && b1 != EMPTY_VALUE && s1 != EMPTY_VALUE);
      if(ok0)
      {
         double eps = InpCrossEqualEps;
         if(eps < 0.0) eps = -eps;
         double diff0 = b0 - s0;
         double diff1 = b1 - s1;
         bool above0 = (diff0 > eps);
         bool below0 = (diff0 < -eps);

         int base_dir = g_state_dir;
         if(diff1 > eps) base_dir = 1;
         else if(diff1 < -eps) base_dir = -1;

         if(above0)
         {
            if(base_dir != 1) st0_buy = 1.0;
            else st0_buy = (diff0 >= diff1 ? 2.0 : 3.0);
         }
         else if(below0)
         {
            if(base_dir != -1) st0_sell = -1.0;
            else st0_sell = (diff0 <= diff1 ? -2.0 : -3.0);
         }
         else
         {
            st0_buy = 4.0;
   ...
   BufStateBuy[0] = st0_buy;
   BufStateSell[0] = st0_sell;
